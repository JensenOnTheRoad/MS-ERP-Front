<template>
  ...
  >
    <el-form-item label="选择商品" prop="id">
      ...
      <el-form-item label="数量" prop="productionQuantity">
        <el-input-number v-model="productionQuantity" :step="1" :min="1"/>
      </el-form-item>

      <el-form-item label="选择仓库" prop="id2">
        <el-select
            v-model="seletedData.warehouse"
            filterable
            placeholder="Select"
        >
          <el-option
              v-for="item in data.options_warehouse"
              :key="item.name"
              :value="item.id"
              :label="item.name"
          />
      ...
      <div v-for="(item, index) in dynamicValidateForm.domains">
        <el-form-item label="选择物资" prop="id">
          <el-select v-model="item.id" filterable placeholder="Select">
            <el-option
                v-for="item_option in data.options_material"
                :key="item_option.name"
                :value="item_option.id"
                :label="
                item_option.name +
...
const url_material_option = global_url + '/production/material/getAll';
const url_warehouse_option = global_url + '/warehouse/warehouse/getAll';
const url_post = global_url + '/production/production-main/insert';
...
const getMaterialOptionData = () => {
  axios
      .get(url_material_option)
      .then(function (response) {
        data.options_material = response.data;
      })
      .catch(function (response) {
        console.log(response);
      });
...
const onSubmit = () => {
  ...
  dynamicValidateForm.domains.forEach((element) => {
    post_data.push({
      ...
      commodityId: seletedData.commodity,
      warehouseId: seletedData.warehouse,
      requiredMaterialId: element.id,
